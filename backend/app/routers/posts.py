...
async def list_posts_admin(
    ...
    # 含 draft：每日自动生成在无图且开启「必须配图」时会写入 draft，否则后台列表为空易被误认为「生成失败」。
    q = coll.where("status", "in", ["published", "scheduled", "draft"])
    if order == "hot":
    ...
    for d in docs:
        ...
        items.append({
            ...
            "type": data.get("type", ""),
            "autoGenerated": bool(data.get("autoGenerated", False)),
            "isOfficialDailyContent": bool(data.get("isOfficialDailyContent", False)),
        })
