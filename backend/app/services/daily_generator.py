...
from app.services.content_quality import (
    ...
    should_reject_by_similarity,
)
from app.services.synthetic_users import pick_random_author
...
def normalize_voice_mode(raw: Any) -> str:
    s = (str(raw).strip().lower() if raw is not None else "casual")
    return s if s in VOICE_MODES else "casual"
...
def content_generation_doc_defaults(raw: dict | None) -> dict:
    ...
    out: dict[str, Any] = {
        ...
        "voiceMode": normalize_voice_mode(r.get("voiceMode")),
...
def load_recent_auto_titles_summaries(*, limit: int = 300) -> tuple[list[str], list[str]]:
    """Pull recent auto-generated posts for cross-batch dedup (title/summary similarity)."""
    titles: list[str] = []
    summaries: list[str] = []
    try:
        q = db.collection("posts").order_by("createdAt", direction=FsQuery.DESCENDING).limit(limit)
        for doc in q.stream():
            d = doc.to_dict() or {}
            if not d.get("autoGenerated"):
                continue
        ...
        logger.warning("load_recent_auto_titles_summaries failed (index?): %s", e)
...
def _voice_mode_prompt_block(voice_mode: str) -> str:
...
async def _generate_with_gemini(
    ...
    *,
    ...
    voice_block = _voice_mode_prompt_block(voice_mode)
...
"""
        try:
            ...
            logger.exception("Gemini generation failed (model=%s)", GEMINI_MODEL_ID)
            return None
...
def _pick_topic_and_style(
    ...
    vm = normalize_voice_mode(voice_mode)
    styles = list(STYLES_FOR_VOICE.get(vm) or STYLES_FOR_VOICE["casual"])
...
async def build_post_dict(
    ...
    *,
    ...
    lang = normalize_language(language)
    voice_mode = normalize_voice_mode(cfg.get("voiceMode"))
    st = normalize_content_style(content_style)
    ...
    if image_required and not has_image:
        ...
        # 直接已发布：按 createdAt / publishedAt 参与最新排序；客户端对 publishedAt>now 会隐藏
        status = "published"

    content_len = len(content)
    generated_by = f"meowcare:daily_generator:{GENERATOR_VERSION}|{GEMINI_MODEL_ID}"
    # 使用 Firestore 合成用户池中的随机用户作为作者（与真实注册账号区分）。
    author_id, author_display, author_avatar = await asyncio.to_thread(pick_random_author, db)
...
async def generate_daily_posts(
    ...
    **_kwargs: Any,
    ...
    cfg["voiceMode"] = normalize_voice_mode(cfg.get("voiceMode"))
    ...
    db_titles, db_summaries = load_recent_auto_titles_summaries()
    recent_titles_pool: list[str] = list(db_titles)
    recent_summaries_pool: list[str] = list(db_summaries)
    title_fingerprints: set[str] = set()
    for t in db_titles:
        fp0 = normalize_title_fingerprint(t)
        if fp0:
            title_fingerprints.add(fp0)
    ...
    for batch_index in range(count):
        topic, content_style = _pick_topic_and_style(selected, recent_pairs, cfg.get("voiceMode"))
        ...
        while attempts < max_attempts_per_slot and not placed:
            ...
            for sub_try in range(3):
                ...
                try:
                    ...
                    doc_final = None
                    break
                title_s = str(doc_try.get("title", ""))
                summary_s = str(doc_try.get("summary", ""))
                content_s = str(doc_try.get("content", ""))
                min_p, max_p = _paragraph_bounds_for_style(st)
                min_len = int(cfg.get("minContentLength", 400))
                ok_q, reason_q = validate_generated_fields(
                    title_s,
                    summary_s,
                    content_s,
                    min_p=min_p,
                    max_p=max_p,
                    min_body_len=min_len,
                )
                if not ok_q:
                    logger.info(
                        "auto post rejected (quality): %s topic=%s style=%s try=%s",
                        reason_q,
                        topic,
                        st,
                        sub_try,
                    ...
                    continue
                sim_rej, sim_r = should_reject_by_similarity(
                    title_s,
                    summary_s,
                    content_s,
                    recent_titles_pool,
                    recent_summaries_pool,
                )
                if sim_rej:
                    logger.info(
                        "auto post rejected (similarity): %s topic=%s style=%s",
                        sim_r,
                        topic,
                        st,
                    )
                    continue
                fp = normalize_title_fingerprint(title_s)
                if fp and fp in title_fingerprints:
                    logger.debug("auto post rejected: fingerprint collision")
                    continue
                doc_final = doc_try
                if fp:
                    title_fingerprints.add(fp)
                recent_titles_pool.append(title_s)
                recent_summaries_pool.append(summary_s)
                break
